...
CONFIG = "ONOS_APPS=gui2,drivers.bmv2,pipelines.basic,segmentrouting,lldpprovider,hostprovider,proxyarp,openflow,odtn-service,optical-rest,roadm"
...
class Topology(Dataplane):
    ...
    def nodes(self):
        ...
        self.add_node("spine_w1", Whitebox("spine_w1", dpid="0000000000000001"))
        self.add_node("leaf_w1", Whitebox("leaf_w1", dpid="0000000000000002"))
        self.add_node("dtn1", SimpleHost("dtn1", ctl=False))

        # Data Center East
        self.add_node("ca1e", Cassini("ca1e"))
        self.add_node("spine_e1", Whitebox("spine_e1", dpid="0000000000000003"))
        self.add_node("leaf_e1", Whitebox("leaf_e1", dpid="0000000000000004"))
    ...
    def gen_fabric_config(self, node, sid, mac, edge):
        config = {
            'devices': {
                'device:{}'.format(node.name): {
                    'segmentrouting': {
                        'name': '{}'.format(node.name),
                        'ipv4NodeSid': sid,
                        'ipv4Loopback': '{}'.format(node.ipctl),
    ...
    def send_config_onos(self):
        ...
        def whitebox_controller(node, ctl, **params):
            n = self.get_node(node)
            c = self.get_node(ctl)
            n.set_controller(c.ipctl, "6653", **params)
        ...
        # West
        whitebox_controller('spine_w1', 'ctl1', seg=True, sid=100, mac='00:00:00:00:01:00', edge=False)
        whitebox_controller('leaf_w1', 'ctl1', seg=True, sid=110, mac='00:00:00:00:01:10', edge=True)

        # East
        whitebox_controller('spine_e1', 'ctl1', seg=True, sid=300, mac='00:00:00:00:03:00', edge=False)
        whitebox_controller('leaf_e1', 'ctl1', seg=True, sid=310, mac='00:00:00:00:03:10', edge=True)
        ...
        stratum_interface("ctl1", "leaf_e1", "dtn3", "2", "10.0.3.254", "24", 10)
