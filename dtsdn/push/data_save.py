...
def statistics(data, time, devs):
    ...
    for _, informations in data.items():
        for info in informations:
            for f_name, env in devs.items():
                if env['pd'] == info['device']:
                    # print(f_name)
...
with InfluxDBClient(url="http://172.19.0.2:8086", token=token, org=org) as client:
    ...
    while True:
        json_payload = []
        # data = dc.get_infos()
        data = onos.get_ports_statistics(ip_ctl)
        # times = data['time']
        times = datetime.now()

        statistics(data, times, devs)
        # write_info()
        # write_statistics()
        # write_links()
        # write_app()
